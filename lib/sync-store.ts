// lib/sync-store.ts
// The cache: one row per client holding the whole dashboard payload, gzipped.
//
...
// A constant, not an env var: changing it is a commit, like the roster.
export const FRESH_WINDOW_MS = 15 * 60 * 1000;

// How long a claimed sync may run before another request may take the lock over.
// Self-healing: a function that dies mid-sync must not freeze the client forever.
...
// wrong row would render client A's dashboard with client B's data — the same
// cross-tenant leak lib/ghl-context.ts exists to prevent — so the caller has to
// have resolved a real client through requireClient() to call these at all.
export async function readSync(
  ...
  // claimSync seeds an empty payload when it takes the lock on a client that was
...
// is how two people opening the same stale client at once produce ONE sync.
//
// The whole decision lives inside the UPDATE's WHERE clause on purpose: doing it
// as read-then-write in TypeScript would leave a window where both callers see it
