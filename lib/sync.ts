// lib/sync.ts
// The GHL sync: everything between "we know which client" and "here is the whole
// dashboard payload". Extracted from app/api/dashboard/route.ts so the route and
// the background refresh call the SAME code — two copies of this would drift at
// the first change, and the background one would drift silently.
//
...
function transformContact(ghl: GHLContact, customFieldMap: Map<string, string>): Contact {
  ...
  // startAfter is GHL's pagination cursor echoed back on every record. It is not
  // data, nothing reads it, and on a 30k-contact account it is ~1 MB of the
  // payload the browser has to download and parse. Dropped here rather than
  // client-side because the cost is the transfer, not the field.
  const { startAfter: _cursor, ...rest } = ghl as GHLContact & { startAfter?: unknown };
  return {
    ...rest,
    name:
      ghl.name?.trim() ||
...
function transformOpportunity(
  ...
  // `relations` is GHL's raw association graph and `sort` is a pagination
  // artifact. Nothing on the client or in lib/ai-tools.ts reads either, neither
  // is in lib/types.ts, and neither is advertised to the assistant — but on a
  // 25k-opportunity account they are ~10 MB of dead weight in the payload.
  const { relations: _rel, sort: _sort, ...rest } = ghl as GHLOpportunity & {
    relations?: unknown;
    sort?: unknown;
  };
...
// Runs one full sync inside the client's credential context and returns the
...
//
// onFrame defaults to a no-op because the background refresh has nobody to report
// progress to: it runs in after(), long after the response was sent.
export async function syncProject(
  client: ClientConfig,
  onFrame: (frame: SyncFrame) => void = () => {},
): Promise<DashboardPayload> {
  return withClient(client, async () => {
    // Structured per-dataset progress. The client renders one live row per step
    // (status + running count), so the user watches every stream advance in
    // parallel instead of staring at a single flickering line. `progress` text
    // frames are still sent as a human-readable fallback.
    const sendStep = (key: string, status: "loading" | "done", count?: number) =>
      onFrame({ type: "step", key, status, ...(count !== undefined ? { count } : {}) });

    onFrame({ type: "progress", message: "Iniciando sincronización…" });
    ...
    onFrame({ type: "progress", message: "Cargando pipelines y configuración…" });
    ...
    onFrame({ type: "progress", message: "Cargando datos de GoHighLevel…" });
    ...
    const [contactsRaw, opportunitiesRaw, pautas, appointments, tasks] =
      await Promise.all([
        getAllContacts((count) => {
          onFrame({ type: "progress", message: `Cargando contactos… ${count.toLocaleString("es-MX")}` });
        ...
        getAllOpportunities((count) => {
          onFrame({ type: "progress", message: `Cargando oportunidades… ${count.toLocaleString("es-MX")}` });
    ...
    onFrame({ type: "progress", message: "Procesando datos…" });
